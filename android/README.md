...
# Base environnement
        ...
        cd tesoro/utils/build_scripts
        docker build -f android32.Dockerfile -t tesoro-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t tesoro-gui-android .
        docker create -it --name tesoro-gui-android tesoro-gui-android bash

# Get the apk

        docker cp tesoro-gui-android:/opt/android/tesoro-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/tesoro-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
