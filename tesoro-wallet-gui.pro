...
!ios:!android {
    LIBS += -L$$WALLET_ROOT/build/lib \
            -L$$WALLET_ROOT/build/external/easylogging++ \
            -L$$WALLET_ROOT/build/external/randomx \
            -L$$WALLET_ROOT/build/external/db_drivers/liblmdb \
            -L$$WALLET_ROOT/build/contrib/epee/src \
            $$LIBS_COMMON
}
...
linux {
   ...
   #     }
    ...
    # currently tesoro has an issue with "static" build and linunwind-dev,
...
# translation stuff
TRANSLATIONS = $$files($$PWD/translations/tesoro-core_*.ts)
...
android{
    deploy.commands += make install INSTALL_ROOT=$$DESTDIR && androiddeployqt --input android-libtesoro-wallet-gui.so-deployment-settings.json --output $$DESTDIR --deployment bundled --android-platform android-21 --jdk /usr/lib/jvm/java-8-openjdk-amd64 -qmldir=$$PWD
}
...
DISTFILES += \
    notes.txt \
    tesoro/src/wallet/CMakeLists.txt
